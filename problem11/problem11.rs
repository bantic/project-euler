...
fn main() {
    ...
    for r in range(0, rows) {
        ...
        for i in range(0, rows - len) {
            let slice = row.slice(i, i+len);
            let prod = slice.iter().fold(1, |a, &b| a * b);
    ...
    for c in range(0,cols) {
        ...
        for r in range(0,rows) {
            col.push( *g.get(r).get(c) );
        }

        for i in range(0, cols - len) {
            let slice = col.slice(i, i+len);
            let prod = slice.iter().fold(1, |a, &b| a*b);
            if prod > max { max = prod; }
        }
    }
    ...
    for c in range(0, cols-len) {
        for r in range(0, rows-len) {
            let mut diag: Vec<int> = vec![];
            for i in range(0, len) {
                diag.push( *g.get(r+i).get(c+i) );
            }
            let prod = diag.iter().fold(1, |a, &b| a*b);
            if prod > max { max = prod; }
        }
    }

    // diag upper right to lower left
    for c in range(len-1, cols) {
        for r in range(0, rows-len) {
            let mut diag: Vec<int> = vec![];
            for i in range(0, len) {
                diag.push( *g.get(r+i).get(c-i) );
            ...
            let prod = diag.iter().fold(1, |a, &b| a*b);
            if prod > max { max = prod; }
            println!("{}x{}: {} -> {}",c,r,diag,prod);
        }
    }

    println!("max prod {}",max);
}
